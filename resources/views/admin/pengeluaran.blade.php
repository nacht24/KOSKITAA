    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
        <div class="p-4 border-b border-gray-100 flex flex-col sm:flex-row items-start sm:items-center gap-3">
            ...
            <div class="sm:ml-auto">
                <a href="#" class="flex items-center gap-1.5 text-xs font-semibold text-gray-500 hover:text-gray-800 transition">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <tbody class="divide-y divide-gray-100 text-sm text-gray-700">
                    ...
                    @forelse($pengeluarans as $p)
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="p-4 text-gray-600 text-xs">{{ $p->deskripsi ?? '-' }}</td>
                            <td class="p-4 font-bold text-gray-900 whitespace-nowrap">Rp {{ number_format($p->nominal, 0, ',', '.') }}</td>
                            <td class="p-4">
                                @if($statusLunas)
                            ...
                            <td class="p-4 text-center">
                                <div class="relative inline-block">
                                    <button onclick="toggleDropdown('drop-{{ $p->id_pengeluaran }}')" class="p-1.5 hover:bg-gray-100 rounded-lg transition text-gray-400 hover:text-gray-700">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                            <circle cx="12" cy="5" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="19" r="1.5"/>
                                        </svg>
                                    </button>
                                    <div id="drop-{{ $p->id_pengeluaran }}" class="hidden absolute right-0 mt-1 w-36 bg-white rounded-xl border border-gray-100 shadow-lg z-20 overflow-hidden">
                                        <button onclick="editData({{ json_encode($p) }}); toggleDropdown('drop-{{ $p->id_pengeluaran }}')" class="flex items-center gap-2 w-full px-4 py-2.5 text-xs font-semibold text-gray-700 hover:bg-gray-50 transition">
                                            <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                            Edit
                                        </button>
                                        <form action="{{ route('admin.pengeluaran.destroy', $p->id_pengeluaran) }}" method="POST" onsubmit="return confirm('Yakin hapus data ini?')">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="flex items-center gap-2 w-full px-4 py-2.5 text-xs font-semibold text-red-500 hover:bg-red-50 transition">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr><td colspan="6" class="p-10 text-center text-gray-400 text-xs">Belum ada data pengeluaran tercatat.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <form id="formPengeluaran" action="{{ route('admin.pengeluaran.store') }}" method="POST">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select name="jenis_pengeluaran" id="jenis_pengeluaran" class="w-full bg-gray-50 border border-gray-200 rounded-xl text-sm px-3 py-2.5 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition" required>
                ...
                <div>
                    ...
                    <input type="text" name="deskripsi" id="deskripsi" placeholder="Contoh: Tagihan Listrik PLN - Oktober" class="w-full bg-gray-50 border border-gray-200 rounded-xl text-sm px-3 py-2.5 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Nominal (Rp)</label>
                    <input type="number" name="nominal" id="nominal" placeholder="0" class="w-full bg-gray-50 border border-gray-200 rounded-xl text-sm px-3 py-2.5 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition" required>
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Status</label>
                    <select name="status" id="status" class="w-full bg-gray-50 border border-gray-200 rounded-xl text-sm px-3 py-2.5 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition">
            ...
            <div class="mt-6 flex gap-3">
                ...
                <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 py-2.5 rounded-xl text-xs font-bold text-white transition shadow-sm shadow-blue-100">Simpan</button>
            </div>
        </form>
    </div>
</div>

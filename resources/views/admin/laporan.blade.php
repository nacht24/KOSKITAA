...
<div class="p-8 max-w-6xl mx-auto">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 relative overflow-hidden">
            ...
            <div class="absolute -right-4 -bottom-4 text-8xl font-black text-gray-50 select-none pointer-events-none leading-none">Rp</div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @forelse($detailPengeluaran as $p)
                        @php
                            ...
                            $idRef  = $p->id_referensi ?? ($prefix . '-' . \Carbon\Carbon::parse($p->created_at)->format('Y') . '-' . str_pad($p->id_pengeluaran, 4, '0', STR_PAD_LEFT));
                        @endphp
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="p-4 text-xs font-mono font-semibold text-gray-500">{{ $idRef }}</td>
                            <td class="p-4 text-xs text-gray-500 whitespace-nowrap">{{ \Carbon\Carbon::parse($p->created_at)->format('d M Y') }}</td>
                            <td class="p-4 text-sm font-bold text-gray-900 whitespace-nowrap">Rp {{ number_format($p->nominal, 0, ',', '.') }}</td>
                            <td class="p-4">
                                @if($statusLunas)
                            ...
                            <td class="p-4 text-center">
                                <div class="relative inline-block">
                                    <button onclick="toggleDrop('ld-{{ $p->id_pengeluaran }}')" class="p-1.5 hover:bg-gray-100 rounded-lg transition text-gray-400 hover:text-gray-700">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                            <circle cx="12" cy="5" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="19" r="1.5"/>
                                        </svg>
                                    </button>
                                    <div id="ld-{{ $p->id_pengeluaran }}" class="hidden absolute right-0 mt-1 w-32 bg-white rounded-xl border border-gray-100 shadow-lg z-20 overflow-hidden">
                                        <a href="{{ route('admin.pengeluaran.index') }}" class="flex items-center gap-2 px-4 py-2.5 text-xs font-semibold text-gray-700 hover:bg-gray-50 transition">
                                            <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                            </svg>
                                            Detail
                                        </a>
                                        <form action="{{ route('admin.pengeluaran.destroy', $p->id_pengeluaran) }}" method="POST" onsubmit="return confirm('Yakin hapus?')">
                                            @csrf @method('DELETE')
        ...
        <div class="p-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p class="text-xs text-gray-400">Menampilkan {{ $detailPengeluaran->firstItem() }}-{{ $detailPengeluaran->lastItem() }} dari {{ $detailPengeluaran->total() }} entri</p>
